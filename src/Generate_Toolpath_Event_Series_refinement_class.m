...
methods
    function generate_event_series_files(obj)
        ...
        % obj.Vel=600;
        obj.Vel=1029;
        %Scan Spacing/ hatch obj.spacing (distance between laser paths in same layer)
        % obj.spa=0.10;
        obj.spa=0.030;
        %length in x of rectangle in mm
        obj.lx=0.6;
        ...
        obj.tlay=0.060;
        %obj.Power
        %obj.Power=120000; %Watts
        obj.Power=375000; %Watts
        %Roller time
        obj.troll=0.2;
        %Time to rest for recoating
        %obj.trest=30.2; 
        obj.trest=1-obj.troll; %substract obj.troll
    ...
    function generate_steps_file(obj)
        ...
        %     %prints rolling step
        %     fprintf(fileID,text,stepN+1,stepN+1,0.001, obj.troll, 0.001, freq,HO_all);
        %     %prints resting step
        %     fprintf(fileID,text,stepN+2,stepN+2,1.0, obj.trest, 1.0, freq,HO_all);
        %     stepN=stepN+3;
        %     
        %     else
        ...
        %     tscan=timemat(4)-timemat(2);
            ...
            %prints rolling step
            fprintf(fileID,text,stepN+1,stepN+1,obj.troll/4, obj.troll, obj.troll/4, freq, HO_all);
            %prints resting step
            fprintf(fileID,text,stepN+2,stepN+2,1.0, obj.trest, 1.0, freq, HO_all);
            stepN=stepN+3;
